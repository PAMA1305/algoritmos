...
btn_save = tk.Button(self.current_frame, text="Guardar",
                     command=self.save_user_data)
btn_save.pack(pady=10)
btn_back = tk.Button(self.current_frame, text="Regresar",
...
self.current_frame = tk.Frame(self.root, bg="lightblue")
self.current_frame.pack(fill=tk.BOTH, expand=True)
label = tk.Label(self.current_frame, text="Avance del Usuario", bg="lightblue",
                 font=("Arial", 18))
label.pack(pady=10)
self.progress_tree = ttk.Treeview(self.current_frame, columns=("Usuario", "Niveles
                                                               Completados"), show='headings')
...
self.current_frame = tk.Frame(self.root, bg="lightblue")
self.current_frame.pack(fill=tk.BOTH, expand=True)
rules_label = tk.Label(self.current_frame, text="Reglas del Juego",
                       bg="lightblue", font=("Arial", 18))
...
rules_display = tk.Label(self.current_frame, text=rules_text, bg="lightblue",
                         justify=tk.LEFT)
...
self.current_frame = tk.Frame(self.root, bg="lightblue")
self.current_frame.pack(fill=tk.BOTH, expand=True)
label = tk.Label(self.current_frame, text="Ingrese su nombre para iniciar el
juego: ", bg="
lightblue
")
label.pack(pady=10)
...
btn_start = tk.Button(self.current_frame, text="Iniciar Juego",
...
for j in range(5):  # 5 columnas
    btn = tk.Button(self.current_frame, text="?", width=5, height=2,
                    command=lambda x=i, y=j: self.flip_card(x, y, user_name,
                                                            level))
...
self.buttons[self.first_card_pos[0]][self.first_card_pos[1]].config(text="?",
                                                                    state="normal")
self.buttons[self.second_card_pos[0]]
[self.second_card_pos[1]].config(text="?", state="normal")
...
if all(btn["text"] != "?" for row in self.buttons for btn in row):
    if level == 1:
        messagebox.showinfo("Nivel Completo", "¡Felicidades! Has completado el
    Nivel
    1.
    ")
    self.level += 1
    ...
    messagebox.showinfo("Juego terminado", f"¡Felicidades, {user_name}! Has
    terminado
    el
    juego.
    ")
